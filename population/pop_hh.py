...
#                self.hh_members(ind))
    ...
    def gen_hh_age_structured_pop(self, pop_size, hh_probs, age_probs_i,     
        ...
        split_probs.reverse()  # this is why hh_comp table, is in the seemingly reverse order, where column 1 is the
        # oldest group
...
#            hh_type = [int(x) for x in sample_uniform(hh_probs, rng)]
    ...
    def allocate_couples(self):
        ...
        for cur_hh in list(self.groups['household'].keys()):
...
#                    print ""
    ...
    def flatten(self):
        ...
        """
        for ind in list(self.I.values()) + list(self.graveyard.values()):
            if ind.partner:
