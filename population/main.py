...
def load_params(params_file='params.cfg'):
...
def run_single(params, store_from=None):
    print("Creating population...")
    sim = Simulation(params)
    # old
    # timesteps = params['years'] * (365/params['t_dur']) # second part will always be floor integer
    timesteps = params['years'] * floor(365/params['t_dur'])
    ...
    for i in range(timesteps):
        ...
        if store_from is not None and i >= store_from:
            sim.save_population(i)
...
def go_single_store_pop(p, store_from, over_ride=True):
    if p['random_seed']:
        p['seed'] = Random().randint(0, 99999999)
    fname = os.path.join(p['prefix'], 'final_%d.sim'%p['seed'])
    if os.path.isfile(fname) and not over_ride:
